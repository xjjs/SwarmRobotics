...
class SAE(object):
    def __init__(self, n_in=30, n_out=2, hidden_layers_sizes=[128, 64, 32, 16]):
        ...
        for i in range(self.n_layers):
            ...
            encoder_layer = HiddenLayer(input = layer_input, n_in = input_size, n_out = hidden_layers_sizes[i], activation = tf.nn.relu)
        ...
        for i in range(self.n_layers):
            ...
            decoder_layer = HiddenLayer(input = layer_input, n_in = input_size, n_out = decoder_layers_sizes[i], activation = tf.nn.relu)
...
while count < 1000000:
    ...
    if count % 200 == 0:
        ...
        if validation_loss < best_validation_loss:
            ...
            with open("parameters-snn-temp83.txt", "w") as outfile:
